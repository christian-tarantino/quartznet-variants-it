...
def test(model, test_loader, criterion, tokenizer, decoder, device, time_major=True):
    ...
    with torch.no_grad():
        for specs, targets, input_lengths, target_lengths in tqdm(test_loader, desc="Testing", leave=False):
            ...
            # Beam Search + KenLM             
            preds_text = decoder.decode_batch(None, batch_logits, beam_width=128) 
...
def main():
    ...
    with open(
        "/home/christian/ai/ASR/unigrams_clean.txt", "r", encoding="utf-8"
    ) as f:
    ...
    print(
        f"Unigrammi filtrati pronti: {len(valid_unigrams)} parole nel vocabolario."
    )

    final_decoder = build_ctcdecoder(
        labels=labels_for_decoder,
        kenlm_model_path=LM_PATH,
        unigrams=valid_unigrams,
        alpha=0.8, #empirically set
        beta=2,    #empirically set
    )

    test_loss, test_wer, test_cer = test(
        model, test_loader, criterion, tokenizer, final_decoder, device
    )
    print(
        f"FINAL TEST RESULTS -> Loss: {test_loss:.4f} | WER: {test_wer:.2f}% | CER: {test_cer:.2f}%"
    )
...
if __name__ == "__main__":
    ...
    MODEL_PTH_PATH="ASR/pth/quartznet_pure_6M.pth"
    



    test_dataset = PackedASRDataset(
        root_dir="/home/christian/ai/ASR/dataset/Common Voice Scripted Speech 26.0 - Italian ¦ Mozilla Data Collective/cv-corpus-26.0-2026-06-12/it",
